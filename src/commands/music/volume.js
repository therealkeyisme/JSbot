module.exports = {
    run: async(client, message, args) => {
        ...
        if(args > 1) {
            message.channel.send("You cannot set the volume that high. you will die").then(message => message.delete({timeout:5000})).catch(err => {throw err});
            return;
        }
        else {
            serverQueue.volume = args; // eslint-disable-line
            serverQueue.connection.dispatcher.setVolumeLogarithmic(args);
            message.channel.send(`I set the volume to: **${args}**`).then(message => message.delete({timeout:5000})).catch(err => {throw err});
            return;
        }
    },
    aliases: [],
